...
// Project identity (user-visible About title + log overlay).
#define BMX8BIT_PROJECT_NAME "BMX8bit"
#define BMX8BIT_PROJECT_VERSION "0.1"
...
static int unit;
// Selected Atari SIO drive (1-8) for the disk image browser.
static int atari_drive_select;
static char atari_uncompr_output[FILENAME_MAX];

int emux_atari_mount_disk(int drive, char *path);
void emux_atari_sync_drives(void);
...
// Atari 8-bit disk images mountable by the SIO core (content-detected, so
// compressed .gz/.atz/.xfz and DCM mount transparently too).
const int num_atari_disk_ext = 9;
static char atari_disk_filt_ext[9][5] = {".atr", ".xfd", ".atx", ".pro",
                                         ".dcm", ".gz", ".atz", ".xfz",
                                         ".zip"};

// Atari disk-set lists (8 lines, TUI-compatible plain text).
const int num_atari_set_ext = 1;
static char atari_set_filt_ext[1][5] = {".ats"};

const int num_tape_ext = 3;
static char tape_filt_ext[3][5] = {".t64", ".tap", ".zip"};

const int num_cart_ext = 3;
static char cart_filt_ext[3][5] = {".crt", ".bin", ".zip"};

const int num_snap_ext = 1;
char snap_filt_ext[1][5];

const int num_prg_ext = 2;
const char prg_filt_ext[2][5] = {".prg", ".zip"};
...
TEST_FILTER_MACRO(test_atari_disk_name, num_atari_disk_ext, atari_disk_filt_ext);
TEST_FILTER_MACRO(test_atari_set_name, num_atari_set_ext, atari_set_filt_ext);

// ZIP-archive browsing (defined below, used by show_files and attach flows).
static int zip_pick_active;
static int zip_depth;
static FileFilter browser_filter;
static char *zip_pick_path(DirType dir, char *name);
// Reboot-persistent trace (weak no-op, overridden by the Atari bridge).
...
static int filter_matches_file(FileFilter filter, char *name) {
  ...
  } else if (filter == FILTER_ATARI_DISK) {
    return test_atari_disk_name(name);
  } else if (filter == FILTER_ATARI_SET) {
    return test_atari_set_name(name);
...
static struct menu_item *show_files(DirType dir_type, FileFilter filter,
  ...
  // Fresh real-browser context: any stale zip pick is void, and no zip
  // level is active (an attach that pops to root leaves zip_depth stale,
  // which would refuse the next descend as "nested").
  zip_pick_active = 0;
  zip_depth = 0;
  ...
  int has_name_field =
      ...
      menu_id == MENU_ATARI_SET_SAVE_FILE ||
      menu_id == MENU_ATARI_BLANK_FILE ||
      menu_id == MENU_ATARI_CART_BLANK_FILE ||
...
static void show_about() {
  ...
  snprintf(title, sizeof(title), "%s %s %s", BMX8BIT_PROJECT_NAME,
           BMX8BIT_PROJECT_VERSION, version);
  ...
  case BMC64_MACHINE_CLASS_PET:
    ...
    break;
  case BMC64_MACHINE_CLASS_ATARI800:
    snprintf(title, sizeof(title), "%s %s %s", BMX8BIT_PROJECT_NAME,
             BMX8BIT_PROJECT_VERSION, version);
    strncpy (desc, "A Bare Metal Atari Emulator", 31);
    break;
  default:
  ...
  ui_menu_add_button(MENU_TEXT, about_root, "For the Raspberry Pi 4/5");

  ui_menu_add_divider(about_root);
  ui_menu_add_button(MENU_TEXT, about_root,
                     "https://github.com/drunkeneye/BMX8bit");
  ui_menu_add_button(MENU_TEXT, about_root, "---------------------------");
}
...
static const struct license_menu_entry license_menu_entries[] = {
  ...
  { MENU_LICENSE_ATARI800, "Atari800", "/licenses/atari800.txt" },
...
static void ui_set_hotkeys() {
  ...
  // VICE hotkey items are not built on the Atari port (own F6..F11 map).
  if (emux_machine_class == BMC64_MACHINE_CLASS_ATARI800) {
    return;
  }
...
static int save_settings() {
  ...
  case BMC64_MACHINE_CLASS_PET:
    ...
    break;
  case BMC64_MACHINE_CLASS_ATARI800:
    fp = fopen("/settings-atari800.txt", "w");
    break;
  default:
  ...
  // VICE CBM/CTRL hotkeys are not built on the Atari port.
  if (emux_machine_class != BMC64_MACHINE_CLASS_ATARI800) {
...
static int load_settings() {
  ...
  case BMC64_MACHINE_CLASS_PET:
    ...
    break;
  case BMC64_MACHINE_CLASS_ATARI800:
    fp = fopen("/settings-atari800.txt", "r");
    break;
  default:
    printf("ERROR: Unhandled machine\n");
    return 0;
  ...
  while (1) {
    ...
    if (port_1_menu_item && strcmp(name, "port_1") == 0) {
      ...
      if (hotkey_cf7_item) hotkey_cf7_item->value = HOTKEY_CHOICE_MENU;
      ...
      if (hotkey_cf1_item) hotkey_cf1_item->value = value;
    } else if (strcmp(name, "hotkey_cf3") == 0) {
      if (hotkey_cf3_item) hotkey_cf3_item->value = value;
    } else if (strcmp(name, "hotkey_cf5") == 0) {
      if (hotkey_cf5_item) hotkey_cf5_item->value = value;
    } else if (strcmp(name, "hotkey_cf7") == 0) {
      if (hotkey_cf7_item) hotkey_cf7_item->value = value;
    } else if (strcmp(name, "hotkey_tf1") == 0) {
      if (hotkey_tf1_item) hotkey_tf1_item->value = value;
    } else if (strcmp(name, "hotkey_tf3") == 0) {
      if (hotkey_tf3_item) hotkey_tf3_item->value = value;
    } else if (strcmp(name, "hotkey_tf5") == 0) {
      if (hotkey_tf5_item) hotkey_tf5_item->value = value;
    } else if (strcmp(name, "hotkey_tf7") == 0) {
      if (hotkey_tf7_item) hotkey_tf7_item->value = value;
...
static void attach_cart(int menu_id, struct menu_item *item) {
  emux_attach_cart(menu_id, zip_pick_path(DIR_CARTS, item->str_value));
}
...
// ZIP archives as folders (extract-on-select). The file browser shows
// .zip rows; picking one pushes a nested browser over the archive's
// entries (filtered like the parent). Picking an entry extracts it to a
// per-zip cache and re-dispatches the original attach flow from there.
// Cache is never deleted mid-session (mounted images stay valid) and is
// wiped once at boot (build_menu).
#define ZIPBROWSE_MAX_DEPTH 4
struct zip_level {
  char zip_abs[256];
  char prefix[256];
};
static struct zip_level zip_stack[ZIPBROWSE_MAX_DEPTH];
static int zip_depth = 0;
static int zip_purpose_id = 0;
static FileFilter zip_filter = FILTER_NONE;

static struct zip_level *zip_top(void) {
  return zip_depth > 0 ? &zip_stack[zip_depth - 1] : NULL;
}
// Filter of the most recently opened real browser (captured in show_files).
static FileFilter browser_filter = FILTER_NONE;
static char zip_pick_abs[256];
static int zip_pick_active = 0;
...
static int zip_is_archive_name(const char *name) {
  const char *dot;
  if (name == NULL) return 0;
  dot = strrchr(name, '.');
  if (dot == NULL) return 0;
  return strcasecmp(dot, ".zip") == 0;
}

// Attach flows eligible for zip descend (result menu IDs).
static int zip_purpose_allows(int id) {
  switch (id) {
  case MENU_DISK_FILE:
  ...
  case MENU_ATARI_DRV_FILE:
  case MENU_ATARI_CART_FILE:
  ...
  // VIC20 / Plus4 cartridge file rows (ranges are consecutive in menu_ids).
  if (id >= MENU_VIC20_CART_DETECT_FILE &&
      id <= MENU_VIC20_CART_FINAL_EXPANSION_FILE)
    return 1;
  if (id >= MENU_PLUS4_CART_FILE && id <= MENU_PLUS4_CART_C2_HI_FILE)
    return 1;
  return 0;
}

// Path override for attach results: an extracted cache file when the pick
// came from inside an archive, else the normal browser path. Consumes the
// flag (fresh browsers reset it; see show_files).
static char *zip_pick_path(DirType dir, char *name) {
  if (zip_pick_active) {
    zip_pick_active = 0;
    return zip_pick_abs;
...
static unsigned zip_fnv1a(const char *s) {
...
static const char *zip_entry_basename(const char *path) {
...
struct zip_list_ctx {
  struct menu_item *dirs;
  struct menu_item *files;
  int purpose_id;
  FileFilter filter;
};

static void zip_list_visit(const char *name, int is_dir, void *ctx) {
  struct zip_list_ctx *lc = (struct zip_list_ctx *)ctx;
  struct zip_level *top = zip_top();
  char full[256];
  struct menu_item *row;
  if (top == NULL) return;
  snprintf(full, sizeof(full), "%s%s", top->prefix, name);
  if (is_dir) {
    row = ui_menu_add_button(lc->purpose_id, lc->dirs, name);
    if (row == NULL) return;
    row->sub_id = MENU_SUB_ENTER_ZIPDIR;
    strncpy(row->str_value, full, MAX_STR_VAL_LEN - 1);
  } else {
    if (!filter_matches_file(lc->filter, (char *)name)) return;
    row = ui_menu_add_button(lc->purpose_id, lc->files,
                             (char *)zip_entry_basename(full));
    if (row == NULL) return;
    row->sub_id = MENU_SUB_PICK_ZIPFILE;
    strncpy(row->str_value, full, MAX_STR_VAL_LEN - 1);
  }
  row->str_value[MAX_STR_VAL_LEN - 1] = '\0';
}

// (Re)build the archive browser at the current prefix. Pops the previous
// level (real browser on descend, zip level on relist) and pushes fresh.
// The reader is opened per level (archives stay validated on every entry,
// no stale handles across emulation).
static void zip_open_level(void) {
  struct menu_item *menu;
  struct zip_list_ctx lc;
  char title[64];
  struct menu_item *up;
  char err[128];
  struct zip_level *top = zip_top();
  ...
  {
    char line[300];
    snprintf(line, sizeof(line), "level %s prefix=%s", top->zip_abs,
             top->prefix);
    zipbrowse_trace(line);
  }
  if (zipbrowse_open(top->zip_abs, err, sizeof(err)) != 0) {
    ui_error(err[0] != '\0' ? err : "Cannot open archive");
    return;
  }
  snprintf(title, sizeof(title), "%s%s", zip_entry_basename(top->zip_abs),
           top->prefix[0] != '\0' ? "/" : "");
  ui_pop_menu();
  menu = ui_push_menu(-1, -1);
  if (menu == NULL) {
    printf("ERROR: cannot show zip browser, menu stack is full\n");
    zipbrowse_close();
    return;
  }
  ui_menu_add_button(MENU_TEXT, menu, title);
  ui_menu_add_divider(menu);
  up = ui_menu_add_button(zip_purpose_id, menu,
                          top->prefix[0] != '\0' ? ".." : "Close archive");
  up->sub_id = MENU_SUB_ZIP_UP;
  ui_menu_add_divider(menu);
  {
    // Buckets like list_files: dirs first, both case-insensitive sorted.
    struct menu_item dirs_root;
    struct menu_item files_root;
    struct menu_item *dfc;
    struct menu_item *ffc;
    int listed;
    memset(&dirs_root, 0, sizeof(dirs_root));
    dirs_root.type = FOLDER;
    dirs_root.is_expanded = 1;
    ...
    lc.dirs = &dirs_root;
    lc.files = &files_root;
    lc.purpose_id = zip_purpose_id;
    lc.filter = zip_filter;
    listed = zipbrowse_list(top->prefix, zip_list_visit, &lc);
    {
      char line[96];
      snprintf(line, sizeof(line), "listed=%d", listed);
      zipbrowse_trace(line);
    }
    if (listed < 0) {
      ui_error("Cannot list archive");
      ui_pop_menu();
    } else {
      if (listed == 0) {
        ui_menu_add_button(MENU_TEXT, menu, "(empty)");
      }
      dfc = dirs_root.first_child;
      merge_sort(&dfc);
      dirs_root.first_child = dfc;
      ffc = files_root.first_child;
      merge_sort(&ffc);
      files_root.first_child = ffc;
      ui_add_all(&dirs_root, menu);
      ui_add_all(&files_root, menu);
    }
  }
  zipbrowse_close();
}

static void zip_descend(struct menu_item *item) {
  DirType dir = menu_file_item_to_dir_index(item);
  struct zip_level *top;
  // Archive picks with PICK_FILE only exist in real (non-zip) browsers, so
  // this is always a fresh descend: drop any stale level left by an attach
  // or menu-exit that popped out of a previous archive instead of refusing
  // as "nested".
  zip_depth = 0;
  top = &zip_stack[zip_depth];
  snprintf(top->zip_abs, sizeof(top->zip_abs), "%s",
           fullpath(dir, item->str_value));
  top->prefix[0] = '\0';
  if (zip_depth == 0) {
    zip_purpose_id = item->id;
    zip_filter = browser_filter;
  }
  zip_depth++;
  ui_info("Opening archive...");
  zip_open_level();
}

// Cache output path for an entry of the current (top) archive:
// <cache>/<sanitized-zip-base>_<fnv>/<internal path>. Best effort mkdir.
static void zip_cache_out(const char *entry, char *out, size_t outsz) {
  struct zip_level *top = zip_top();
  char cache_root[256];
  char vol[8];
  char *sep;
  char zbase[64];
  size_t i, n;
  DirType dir = DIR_SNAPS;
  if (top == NULL || out == NULL || outsz == 0) return;
  // Cache lives beside the snapshots; volume follows the archive. The
  // snapshots dir type resolves volumes the same way file flows do.
  snprintf(vol, sizeof(vol), "%s", current_volume_names[dir]);
  snprintf(cache_root, sizeof(cache_root), "%s%s%s/.zcache", vol,
           default_dir_names[DIR_SNAPS], machine_sub_dir);
  // <sanitized-zip-basename>_<fnv hash> isolates archives.
  sep = strrchr(top->zip_abs, '/');
  snprintf(zbase, sizeof(zbase), "%s", sep != NULL ? sep + 1 : top->zip_abs);
  for (i = 0, n = 0; zbase[i] != '\0' && n < sizeof(zbase) - 12; i++) {
    char c = zbase[i];
    if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') ||
        (c >= '0' && c <= '9')) {
      zbase[n++] = c;
    } else if (n > 0 && zbase[n - 1] != '_') {
      zbase[n++] = '_';
    }
  }
  zbase[n] = '\0';
  snprintf(out, outsz, "%s/%s_%08x/%s", cache_root, zbase,
           zip_fnv1a(top->zip_abs), entry);
  // mkdir chain for the entry's parent dirs.
  {
    char parent[256];
    char *last;
    ...
    if (last != NULL) {
      *last = '\0';
      zipbrowse_mkdir_chain(parent);
    }
  }
}

// Extract an entry of the current archive to the cache. Returns 0 with
// the output path in out[].
static int zip_do_extract(const char *entry, char *out, size_t outsz) {
  struct zip_level *top = zip_top();
  char err[128];
  int rc;
  if (top == NULL) return -1;
  zip_cache_out(entry, out, outsz);
  ui_info("Extracting...");
  if (zipbrowse_open(top->zip_abs, err, sizeof(err)) != 0) {
    ui_error(err[0] != '\0' ? err : "Cannot open archive");
    return -1;
  }
  rc = zipbrowse_extract(entry, out, err, sizeof(err));
  zipbrowse_close();
  if (rc != 0) {
    ui_error(err[0] != '\0' ? err : "Extract failed");
    return -1;
  }
  return 0;
}

// Extract the picked entry into the per-zip cache and arm the path
// override for the attach flow. Returns 0 on success.
static int zip_extract_pick(struct menu_item *item) {
  char out[256];
  if (zip_do_extract(item->str_value, out, sizeof(out)) != 0) {
    return -1;
  }
  snprintf(zip_pick_abs, sizeof(zip_pick_abs), "%s", out);
  zip_pick_active = 1;
  return 0;
}
...
static void select_file(struct menu_item *item) {
  ...
  if (item->sub_id == MENU_SUB_PICK_ZIPFILE) {
    // Pick inside an archive: a nested .zip descends (extract + push a
    // level), anything else extracts and runs the normal attach flow
    // for the original purpose from the cache file.
    if (zip_is_archive_name(item->str_value)) {
      char out[256];
      struct zip_level *top;
      if (zip_depth >= ZIPBROWSE_MAX_DEPTH) {
        ui_info("Archive nesting too deep");
        return;
      }
      if (zip_do_extract(item->str_value, out, sizeof(out)) != 0) {
        return;
      ...
      top = &zip_stack[zip_depth];
      snprintf(top->zip_abs, sizeof(top->zip_abs), "%s", out);
      top->prefix[0] = '\0';
      zip_depth++;
      zip_open_level();
      return;
    }
    if (zip_extract_pick(item) < 0) {
      return;
    }
  ...
  if (item->sub_id == MENU_SUB_ENTER_ZIPDIR) {
    struct zip_level *top = zip_top();
    if (top == NULL) return;
    snprintf(top->prefix, sizeof(top->prefix), "%s/", item->str_value);
    zip_open_level();
    return;
  }
  if (item->sub_id == MENU_SUB_ZIP_UP) {
    struct zip_level *top = zip_top();
    if (top != NULL && top->prefix[0] != '\0') {
      // Up one internal level: strip the last component.
      size_t len = strlen(top->prefix);
      if (len > 0 && top->prefix[len - 1] == '/') {
        top->prefix[len - 1] = '\0';
      }
      {
        char *slash = strrchr(top->prefix, '/');
        if (slash != NULL) {
          slash[1] = '\0';
        } else {
          top->prefix[0] = '\0';
      ...
      // Up out of a nested archive into its parent level.
      zip_depth--;
      zip_open_level();
    } else {
      zip_depth = 0;
  ...
  if (item->type != TEXTFIELD && item->sub_id == MENU_SUB_PICK_FILE &&
      zip_purpose_allows(item->id) &&
      zip_is_archive_name(item->str_value)) {
    zip_descend(item);
    return;
  }
  ...
  switch (item->id) {
     ...
     case MENU_LOAD_REU_FILE:
       ...
       return;
     case MENU_DEFAULT_DISK_FILE:
       default_disk_set_image(zip_pick_path(DIR_DISKS, item->str_value));
       ui_pop_menu();
       return;
     case MENU_DISK_FILE:
       // Perform the attach
       ui_info("Attaching...");
       if (emux_attach_disk_image(unit, zip_pick_path(DIR_DISKS, item->str_value)) <
           0) {
         ui_pop_menu();
	 ...
	 strcpy (attached_disk_name[unit-8], item->str_value);
       ...
       return;
     case MENU_ATARI_DRV_FILE:
       // Atari insert stays in the menu (multi-drive workflow). From
       // inside an archive, relist the archive level instead of popping.
       {
         int from_zip = zip_pick_active;
         char attached_msg[64];
         if (emux_atari_mount_disk(atari_drive_select,
                                   zip_pick_path(DIR_DISKS,
                                                 item->str_value)) < 0) {
           if (!from_zip) {
             ui_pop_menu();
           }
           ...
           snprintf(attached_msg, sizeof(attached_msg), "Attached D%d",
                    atari_drive_select);
           if (from_zip) {
             zip_open_level();
           } else {
           ...
           ui_info(attached_msg);
         }
       }
       emux_atari_sync_drives();
       return;
     case MENU_ATARI_SET_SAVE_FILE:
       if (emux_atari_save_set(fullpath(DIR_SNAPS, item->str_value)) < 0) {
         ui_pop_menu();
         ui_error("Failed to save disk set");
       } else {
         ui_pop_menu();
         ui_info("Disk set saved");
       }
       return;
     case MENU_ATARI_SET_LOAD_FILE:
       if (emux_atari_load_set(fullpath(DIR_SNAPS, item->str_value)) < 0) {
         ui_pop_menu();
         ui_error("Failed to load disk set");
       } else {
         ui_pop_menu();
         ui_info("Disk set loaded");
       }
       emux_atari_sync_drives();
       return;
     case MENU_ATARI_BLANK_FILE:
       if (emux_atari_blank_atr(fullpath(DIR_DISKS, item->str_value)) < 0) {
         ui_pop_menu();
         ui_error("Failed to create disk image");
       } else {
         ui_pop_menu();
         ui_info("Blank disk created");
       }
       return;
     case MENU_ATARI_CART_BLANK_FILE:
       if (emux_atari_blank_cart(fullpath(DIR_CARTS, item->str_value)) < 0) {
         ui_pop_menu();
         ui_error("Failed to create cartridge");
       } else {
         ui_pop_menu();
         ui_info("Blank cartridge created");
       }
       return;
     case MENU_ATARI_UNCOMPRESS_FILE:
       if (emux_atari_uncompress(fullpath(DIR_DISKS, item->str_value),
                                 atari_uncompr_output,
                                 sizeof(atari_uncompr_output)) < 0) {
         ui_pop_menu();
         ui_error("Cannot convert this file");
       } else {
         char converted_msg[MAX_STR_VAL_LEN + 32];
         ui_pop_menu();
         snprintf(converted_msg, sizeof(converted_msg), "Wrote %s",
                  atari_uncompr_output);
         ui_info(converted_msg);
       }
       return;
     ...
     case MENU_TAPE_FILE:
       ui_info("Attaching...");
       if (emux_attach_tape_image(zip_pick_path(DIR_TAPES, item->str_value)) < 0) {
         ui_pop_menu();
     ...
     case MENU_AUTOSTART_FILE:
       ui_info("Starting...");
       if (emux_autostart_file(zip_pick_path(DIR_DISKS, item->str_value), 0) < 0) {
         ui_pop_menu();
         ui_error("Failed to autostart file");
       } else {
         ui_pop_all_and_toggle();
     ...
     case MENU_LOADPRG_FILE:
       ui_info("Loading...");
       if (emux_autostart_file(zip_pick_path(DIR_ROOT, item->str_value), 0) < 0) {
         ui_pop_menu();
...
static struct menu_item *relist_files_after_dir_change(int menu_id) {
  ...
  case MENU_ATARI_SET_SAVE_FILE:
  case MENU_ATARI_SET_LOAD_FILE:
    return show_files(DIR_SNAPS, FILTER_ATARI_SET, menu_id, 1);
  case MENU_DEFAULT_DISK_FILE:
  case MENU_DISK_FILE:
  case MENU_CREATE_D64_FILE:
  case MENU_CREATE_D67_FILE:
  ...
  case MENU_ATARI_BLANK_FILE:
  case MENU_ATARI_UNCOMPRESS_FILE:
    return show_files(DIR_DISKS, FILTER_ATARI_DISK, menu_id, 1);
  case MENU_TAPE_FILE:
  case MENU_CREATE_TAP_FILE:
    return show_files(DIR_TAPES,
                      emux_machine_class == BMC64_MACHINE_CLASS_ATARI800
                          ? FILTER_NONE
                          : FILTER_TAPE,
                      menu_id, 1);
...
static void menu_value_changed(struct menu_item *item) {
  ...
  case MENU_ATTACH_DISK_10:
  case MENU_ATTACH_DISK_11:
    // Atari disk images (.atr/.xfd/.atx/...) are not in the VICE disk
    // filter; show everything on the Atari machine.
    show_files(DIR_DISKS,
               ...
               MENU_DISK_FILE, 0);
    return;
  case MENU_ATARI_DRV_INS_1:
  case MENU_ATARI_DRV_INS_2:
  case MENU_ATARI_DRV_INS_3:
  case MENU_ATARI_DRV_INS_4:
  case MENU_ATARI_DRV_INS_5:
  case MENU_ATARI_DRV_INS_6:
  case MENU_ATARI_DRV_INS_7:
  case MENU_ATARI_DRV_INS_8:
    atari_drive_select = item->id - MENU_ATARI_DRV_INS_1 + 1;
    show_files(DIR_DISKS, FILTER_ATARI_DISK, MENU_ATARI_DRV_FILE, 1);
    return;
  case MENU_ATARI_SET_SAVE:
    show_files(DIR_SNAPS, FILTER_ATARI_SET, MENU_ATARI_SET_SAVE_FILE, 1);
    return;
  case MENU_ATARI_SET_LOAD:
    show_files(DIR_SNAPS, FILTER_ATARI_SET, MENU_ATARI_SET_LOAD_FILE, 1);
    return;
  case MENU_ATARI_BLANK:
    show_files(DIR_DISKS, FILTER_ATARI_DISK, MENU_ATARI_BLANK_FILE, 1);
    return;
  case MENU_ATARI_UNCOMPRESS:
    show_files(DIR_DISKS, FILTER_ATARI_DISK, MENU_ATARI_UNCOMPRESS_FILE, 1);
    return;
  ...
  case MENU_ATTACH_TAPE:
    // Atari cassettes (.cas) are not in the VICE tape filter.
    show_files(DIR_TAPES,
               emux_machine_class == BMC64_MACHINE_CLASS_ATARI800
                   ? FILTER_NONE
                   : FILTER_TAPE,
               MENU_TAPE_FILE, 0);
    return;
  case MENU_ATARI_ATTACH_CART:
    show_files(DIR_CARTS, FILTER_NONE, MENU_ATARI_CART_FILE, 0);
    return;
  case MENU_ATARI_CART_BLANK:
    show_files(DIR_CARTS, FILTER_NONE, MENU_ATARI_CART_BLANK_FILE, 1);
    return;
  ...
  if (item->sub_id == MENU_SUB_PICK_FILE || item->sub_id == MENU_SUB_PICK_DIR) {
    select_file(item);
    return;
  } else if (item->sub_id == MENU_SUB_PICK_ZIPFILE ||
             item->sub_id == MENU_SUB_ENTER_ZIPDIR ||
             item->sub_id == MENU_SUB_ZIP_UP) {
    select_file(item);
    return;
...
static BMC64MachineClass machine_emulator_class(MachineEmulator emulator) {
  switch (emulator) {
    ...
    case MACHINE_EMULATOR_ATARI800:
      return BMC64_MACHINE_CLASS_ATARI800;
    default:
...
static const char *machine_emulator_name(MachineEmulator emulator) {
  switch (emulator) {
    ...
    case MACHINE_EMULATOR_ATARI800: return "atari800";
    default: return "Unknown";
  }
}
...
static MachineEmulator current_machine_emulator(void) {
  switch (emux_machine_class) {
    ...
    case BMC64_MACHINE_CLASS_ATARI800: return MACHINE_EMULATOR_ATARI800;
...
void build_menu(struct menu_item *root) {
  ...
  case BMC64_MACHINE_CLASS_PET:
    ...
    break;
  case BMC64_MACHINE_CLASS_ATARI800:
    strcat(machine_info_txt,"Atari 800 ");
    strcpy(machine_sub_dir, "/ATARI800");
    break;
  default:
  ...
  // Fresh boot = no mounted images: wipe extracted archive caches on all
  // volumes (never deleted mid-session while mounts may reference them).
  {
    static const char *zip_vols[] = { user_volume_name, system_volume_name };
    size_t v;
    for (v = 0; v < sizeof(zip_vols) / sizeof(zip_vols[0]); v++) {
      char cache[128];
      snprintf(cache, sizeof(cache), "%s%s%s/.zcache", zip_vols[v],
               default_dir_names[DIR_SNAPS], machine_sub_dir);
      zipbrowse_wipe_dir(cache);
    }
  }
  ...
  {
    char about_label[36];
#ifdef BMX_BUILD_STAMP
    snprintf(about_label, sizeof(about_label), "About (%s)", BMX_BUILD_STAMP);
#else
    snprintf(about_label, sizeof(about_label), "About...");
#endif
    ui_menu_add_button(MENU_ABOUT, root, about_label);
  }
  ...
  switch (emux_machine_class) {
    ...
    case BMC64_MACHINE_CLASS_PET:
     break;
    default:
     ui_menu_add_button(MENU_AUTOSTART, root,
         emux_machine_class == BMC64_MACHINE_CLASS_ATARI800 ?
         "Autostart..." : "Autostart Prg/Disk...");
  ...
  if (emux_machine_class == BMC64_MACHINE_CLASS_ATARI800) {
    // Atari SIO drives D1:-D8: plus disk-set/rotate/blank management are
    // built by the bridge (emux_add_drive_option with drive -1 above);
    // the VICE drive model/IEC/default-disk UI does not apply.
  } else {
  ...
  if (emux_machine_class == BMC64_MACHINE_CLASS_ATARI800) {
     ui_menu_add_button(MENU_ATARI_SCREENSHOT, video_parent,
                     "Save screenshot (.bmp)");
  }
  ...
  struct menu_item *sound_sid_parent = NULL;
  if (emux_machine_class != BMC64_MACHINE_CLASS_ATARI800) {
    sound_sid_parent = ui_menu_add_folder(sound_parent, "SID");
  }
  ...
  // The mapping editor (machine maps, ISO layouts) and the CBM/CTRL
  // function-key hotkeys are VICE concepts: hidden on the Atari port, which
  // maps F6..F11 in its own keyboard options instead. Monitor stays.
  if (emux_machine_class != BMC64_MACHINE_CLASS_ATARI800) {
  ...
  }  // !ATARI800 (CBM/CTRL hotkeys are VICE concepts)
  ...
  struct menu_item *update_item =
      ui_menu_add_button(MENU_SYSTEM_UPDATE, parent, "Update...");
  // No update channel in this fork (see README): listed but disabled.
  update_item->disabled = 1;
  if (emux_update_draft_test_available() > 0) {
    struct menu_item *draft_item = ui_menu_add_button(
        MENU_SYSTEM_UPDATE_DRAFT, parent, "Test prepared draft...");
    draft_item->disabled = 1;
  }
  {
    char developer_password[BMX_DEVELOPER_PASSWORD_MAX_LEN + 1] = "";
    ...
    // Developer mode is unsupported in this fork (errors when enabled):
    // list the submenu greyed out like System Update.
    developer_status_item->disabled = 1;
    ...
    developer_password_item->disabled = 1;
    ...
    developer_buffer_size_item->disabled = 1;
