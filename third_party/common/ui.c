...
static const char *update_progress_title = "BMX8bit Update";

// Operation-specific overlay title ("Extracting..." for archive work).
// NULL or empty restores the default. Call while the session is active;
// it only affects the next render.
void ui_update_progress_set_title(const char *title) {
  if (title != NULL && title[0] != '\0') {
    update_progress_title = title;
  } else {
    update_progress_title = "BMX8bit Update";
  }
}
...
static void ui_render_update_progress(void) {
  ...
  ui_draw_text(update_progress_title, left + 2 * 8, top + 8, FG_COLOR);
  ...
  if (update_progress_cancel_requested) {
    ...
    ui_draw_text("Do not turn off BMX8bit", left + 2 * 8,
                 top + 6 * 8, FG_COLOR);
  }
}
...
int ui_update_progress_begin(void) {
  ...
  update_progress_title = "BMX8bit Update";
  update_progress_phase = 0U;  update_progress_per_mille = 0U;
...
void ui_find_first(char letter) {
  // Type-ahead: letters typed within ~1s accumulate into a prefix; the
  // cursor jumps to the first item starting with it. A lone letter (or a
  // prefix matching nothing) keeps the classic single-letter cycling.
  static char prefix[20];
  static int prefix_len = 0;
  static uint32_t prefix_ticks = 0;
  uint32_t now = (uint32_t)circle_get_ticks();

  int menu_index = current_menu;
  int start_index = menu_cursor[current_menu];

  letter = (char)tolower((unsigned char)letter);
  if (prefix_len > 0 && (uint32_t)(now - prefix_ticks) < 1000000U) {
    if (prefix_len < (int)sizeof(prefix) - 1) {
      prefix[prefix_len++] = letter;
      prefix[prefix_len] = '\0';
    }
  } else {
    prefix[0] = letter;
  ...
  while(current_menu == menu_index) {
    ...
    if (name[0] != '\0' && strncasecmp(name, prefix, prefix_len) == 0) break;
  }

  // Multi-letter prefix with no match anywhere: fall back to classic
  // single-letter cycling so fast repeated letters keep working.
  if (prefix_len > 1 && current_menu == menu_index &&
      menu_cursor[current_menu] == start_index) {
    prefix[0] = letter;
    prefix[1] = '\0';
    prefix_len = 1;
    while(current_menu == menu_index) {
