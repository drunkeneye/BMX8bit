...
if (static_kernel->circle_get_ticks() - entry_start >= entry_delay) {
  ...
  if ((usb_pref == USB_PREF_HAT || usb_pref == USB_PREF_HAT_AND_PADDLES) &&
    ...
    // Analog fallback: a stick may live on axes while an unused hat is
    // reported and the HAT preference is selected. OR the axes in so a
    // wrong preference can't silence the stick. Skipped when axes drive
    // paddles. Floor deadzone at 15% rejects axis noise.
    int ax_up = 0, ax_down = 0, ax_left = 0, ax_right = 0;
    if (usb_pref == USB_PREF_HAT && pState->naxes > max_index) {
      float ftx = thresh_x < 0.15f ? 0.15f : thresh_x;
      float fty = thresh_y < 0.15f ? 0.15f : thresh_y;
      ...
      int tx = (maxx - minx) / 2 * ftx;
      int mx = (maxx + minx) / 2;
      int ty = (maxy - miny) / 2 * fty;
      int my = (maxy + miny) / 2;
      ax_left = pState->axes[axis_x].value < mx - tx;
      ax_right = pState->axes[axis_x].value > mx + tx;
      ax_up = pState->axes[axis_y].value < my - ty;
      ax_down = pState->axes[axis_y].value > my + ty;
    }
    bool has_changed =
        (prev_buttons[nDeviceIndex] != b) || (prev_dpad[nDeviceIndex] != dpad) ||
        (prev_axes_dirs[nDeviceIndex][0] != ax_up) ||
        (prev_axes_dirs[nDeviceIndex][1] != ax_down) ||
        (prev_axes_dirs[nDeviceIndex][2] != ax_left) ||
        (prev_axes_dirs[nDeviceIndex][3] != ax_right);
    ...
    if (has_changed) {
      ...
      int old_ax_up = prev_axes_dirs[nDeviceIndex][0];
      int old_ax_down = prev_axes_dirs[nDeviceIndex][1];
      int old_ax_left = prev_axes_dirs[nDeviceIndex][2];
      int old_ax_right = prev_axes_dirs[nDeviceIndex][3];
      ...
      if (ui_activated) {
        ...
        if (ax_up && !old_ax_up) {
          emu_ui_key_interrupt(KEYCODE_Up, 1);
        } else if (!ax_up && old_ax_up) {
          emu_ui_key_interrupt(KEYCODE_Up, 0);
        }
        if (ax_down && !old_ax_down) {
          emu_ui_key_interrupt(KEYCODE_Down, 1);
        } else if (!ax_down && old_ax_down) {
          emu_ui_key_interrupt(KEYCODE_Down, 0);
        }
        if (ax_left && !old_ax_left) {
          emu_ui_key_interrupt(KEYCODE_Left, 1);
        } else if (!ax_left && old_ax_left) {
          emu_ui_key_interrupt(KEYCODE_Left, 0);
        }
        if (ax_right && !old_ax_right) {
          emu_ui_key_interrupt(KEYCODE_Right, 1);
        } else if (!ax_right && old_ax_right) {
          emu_ui_key_interrupt(KEYCODE_Right, 0);
        }
    ...
    // Hat fallback (mirror of the analog fallback above): a stick may
    // live on the hat while ANALOG is selected.
    int hatdpad = pState->nhats > 0 ? pState->hats[0] : 8;
    bool has_changed = (prev_buttons[nDeviceIndex] != b) ||
                       ...
                       (prev_dpad[nDeviceIndex] != hatdpad);
    if (has_changed) {
      ...
      int old_hatdpad = prev_dpad[nDeviceIndex];
      prev_axes_dirs[nDeviceIndex][0] = a_up;
      prev_axes_dirs[nDeviceIndex][1] = a_down;
      prev_axes_dirs[nDeviceIndex][2] = a_left;
      prev_axes_dirs[nDeviceIndex][3] = a_right;
      ...
      if (ui_activated) {
        if (hatdpad == 0 && old_hatdpad != 0) {
          emu_ui_key_interrupt(KEYCODE_Up, 1);
        } else if (hatdpad != 0 && old_hatdpad == 0) {
          emu_ui_key_interrupt(KEYCODE_Up, 0);
        }
        if (hatdpad == 4 && old_hatdpad != 4) {
          emu_ui_key_interrupt(KEYCODE_Down, 1);
        } else if (hatdpad != 4 && old_hatdpad == 4) {
          emu_ui_key_interrupt(KEYCODE_Down, 0);
        }
        if (hatdpad == 6 && old_hatdpad != 6) {
          emu_ui_key_interrupt(KEYCODE_Left, 1);
        } else if (hatdpad != 6 && old_hatdpad == 6) {
          emu_ui_key_interrupt(KEYCODE_Left, 0);
        }
        if (hatdpad == 2 && old_hatdpad != 2) {
          emu_ui_key_interrupt(KEYCODE_Right, 1);
        } else if (hatdpad != 2 && old_hatdpad == 2) {
          emu_ui_key_interrupt(KEYCODE_Right, 0);
        }
        if (a_up && !prev_a_up) {
          emu_ui_key_interrupt(KEYCODE_Up, 1);
