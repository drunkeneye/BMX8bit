...
#if defined(RASPI_ATARI800)
#include "atari800/atari800_emulator_core.h"
#else
...
public:
  ...
  ViceApp(const char *kernel)
      : FromKernel(kernel)
#if RASPPI == 5
        // Circle defaults Pi 5 serial to the dedicated JST UART (device 10).
...
public:
  ViceScreenApp(const char *kernel)
     ...
     mEmulatorCore =
#if defined(RASPI_ATARI800)
         new Atari800EmulatorCore(&mMemory, circle_cycles_per_second());
#else
         new ViceEmulatorCore(&mMemory, circle_cycles_per_second());
#endif
...
protected:
  ...
  CEMMCDevice mEMMC;
  FATFS mFileSystemSYS;
  FATFS mFileSystemUSER;
  FATFS mFileSystemSD;
