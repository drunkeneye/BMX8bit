...
ensure_toolchain() {
  ...
  if [ "$toolchain_complete" -eq 0 ]; then
    ...
    # A truncated download still extracts partially and leaves the directory
    # behind, which would skip re-downloading forever and fail cryptically
    # much later (e.g. missing cc1plus in circle-stdlib configure). Verify
    # the compiler backend and retry once from a clean slate.
    if [ ! -x "$("$TOOLCHAIN_DIR/bin/${TOOL_PREFIX}g++" -print-prog-name=cc1plus 2>/dev/null)" ]; then
      echo "incomplete cross toolchain detected, re-downloading" >&2
      rm -rf -- "$TOOLCHAIN_DIR" "$TOOLCHAIN_ROOT/$TOOLCHAIN_TARBALL"
      curl -L --fail "$TOOLCHAIN_URL" -o "$TOOLCHAIN_ROOT/$TOOLCHAIN_TARBALL"
      tar -C "$TOOLCHAIN_ROOT" -xf "$TOOLCHAIN_ROOT/$TOOLCHAIN_TARBALL"
    fi
...
vice310_machine_config() {
  ...
  case "$machine" in
    ...
    atari800)
      # atari800 is compiled directly from vendored sources in the main
...
build_vice310_link_archives() {
  ...
  if [ "${#targets_by_directory[@]}" -eq 0 ]; then
    return 0
  fi
