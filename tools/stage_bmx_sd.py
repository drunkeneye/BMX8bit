...
def source_overrides(context: StageContext, generated: Path) -> dict[str, Path]:
    ...
    # User media (carts/disks/tapes/prg trees) lives outside the repo so
    # binary blobs never enter git. BMX_MEDIA_DIR overrides the default,
    # which is the repository's parent directory (e.g. .../code/carts/).
    # Absent subdirectories are skipped silently (optional tree rules).
    media_root = os.environ.get("BMX_MEDIA_DIR")
    return {
        "repo": REPO_ROOT,
        "kernels": context.kernel_dir,
        "generated": generated,
        "roms": REPO_ROOT / "third_party/vice-3.10/data",
        "usermedia": Path(media_root) if media_root else REPO_ROOT.parent,
    }
