...
`build_piX.sh` fetches missing ROMs automatically before staging
(`--download-roms`, on by default; `--no-download-roms` disables it):
VICE ROMs from zimmers.net (everything except the SuperCPU binary) into
`roms/<machine>/` (promoted into `third_party/vice-3.10/data/`), Atari ROMs
from FW-Altirra into `roms/a800/` (staged 1:1 to `/roms`). Present files are
never re-downloaded.
